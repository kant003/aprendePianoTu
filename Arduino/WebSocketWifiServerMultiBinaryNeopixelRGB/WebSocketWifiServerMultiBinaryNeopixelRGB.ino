...
void readBinary(WebsocketsMessage msg)
{
  ...
  for (int i = 0; i < msg.rawData().length(); i=i+5){
      ...
      if(binaryData[i] == 255) strip.setBrightness(binaryData[i+1]);
      else{
      uint32_t c = strip.Color(binaryData[i+2], binaryData[i+3], binaryData[i+4]);
      // pixelNumber + 255 * dispositiveNumber
      strip.setPixelColor(binaryData[i+1]+256*binaryData[i], c);
      }
    }
